...
namespace RSBingoBot.Component_interaction_handlers
{
    ...
    public class ChangeTileButtonHandler : ComponentInteractionHandler
    {
        ...
        private void UpdateToTileInfo(ComponentInteractionCreateEventArgs args)
        {
            ...
            if (!toTileInfo.SelectComponent.SelectedItems.Any())
            {
                errorMessage += "Must select a tile to change to.\n";
            }
            else
            {
                if (toTileInfo.SelectComponent.SelectedItems[0].value == null)
                {
                    errorMessage += "An internal error has occurred when retrieving the task to change to.\n";
                }
                else
                {
                    ...
                    if (toTileInfo.TaskAtSelection == default)
                    {
                        errorMessage += "The tile to change from cannot be found, it has likely been deleted.\n";
                    }
                    else
                    {
                        ...
                        if (toTileInfo.Tile != default)
                        {
                            toTileInfo.BoardIndexAtSelection = toTileInfo.Tile.BoardIndex;
                        }
                    }
                }
            }
        ...
        private struct FromTileSelectValue
        {
