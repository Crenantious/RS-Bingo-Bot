...
public class CommandController : ApplicationCommandModule
{
    ...
    private const string TasksFileName = "Tasks.csv";
    ...
    private async Task<string?> AddOrDeleteTasks(InteractionContext ctx, DiscordAttachment attachment, bool addTasks)
    {
        if (!HasAdminPermission(ctx))
        {
            await InsufficientPermissionsResponse(ctx);
        ...
        if (attachment.FileName.EndsWith(".csv") is false)
        {
            return "Incorrect file type; must be csv.";
        }

        try
        {
            DownloadFile(attachment.Url, TasksFileName);
        }
        catch (Exception e)
        {
